...
%A = A(2:end,:); % action is saccade destination position 
...
if depict
    ...
    %%{
    plot(tgt_px_fx, tgt_py_fx, 'bx', 'MarkerSize',10,'LineWidth',2)
    hold on;
    plot(tgt_px_lo, tgt_py_lo, 'bv', 'MarkerSize',10,'LineWidth',2);
    plot(tgt_px_hi, tgt_py_hi, 'b^', 'MarkerSize',10,'LineWidth',2);
    %}
    ...
    grid on;
    legend(...
        'fix tgt', 'lo tgt', 'hi tgt', ...
        'eye p', 'eye start', 'eye end', ...
    ...
    plot(t, tgt_px_fx, '--r'); hold on; plot(t, tgt_py_fx, '--b');
    plot(t, tgt_px_hi,  '-r'); hold on; plot(t, tgt_py_hi,  '-b');
    plot(t, tgt_px_lo,  ':r'); hold on; plot(t, tgt_py_lo,  ':b');
    plot(t, eye_px_filt_trl, 'Color', [1,.5,.5]); plot(t, eye_py_filt_trl, 'Color', [.5,.5,1]);
    grid on;
    legend('tgt x fix', 'tgt y fix', 'tgt x hi', 'tgt y hi', 'tgt x lo', 'tgt y lo', ...
        'eye x', 'eye y', 'Location','eastoutside');
