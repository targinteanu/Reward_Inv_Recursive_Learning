...
MDP = createMDP(length(Sspace), length(Aspace)); 
for s = 1:length(Sspace)
    S = Sspace(s); S = ungridState(S);
    pos_old = [S.eye_px_filt_trl, S.eye_py_filt_trl]; 
    tgt_old = [S.tgt_px_fx, S.tgt_py_fx; 
               S.tgt_px_lo, S.tgt_py_lo; 
               S.tgt_px_hi, S.tgt_py_hi];
